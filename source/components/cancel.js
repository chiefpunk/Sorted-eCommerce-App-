    ...
    _retrieveData = async () => {
        try {
          ...
          if (token !== null) {
            ...
            var temp="https://afandim.net/merchantapp/api/loadCancelOrder?token="+this.state.user_token+"&user_type="+this.state.user_info['user_type']+"&mtid="+this.state.user_info['merchant_id']+"&lang=en&api_key=2bf5c1669378eeda07ed05b3a8e574ef&json=true";
                    ...
                    this.state.offer_data.map((offer, idx) => (
                        <ListItem avatar>
                            ...
                            <Body>
                                ...
                                <View style={{flexDirection:'row'}}>
                                    <Left>
                                        <Text style={{backgroundColor:'gray', width:80, textAlign:'center', color:'white'}}>{offer.status}</Text>
                                    </Left>
                                    <Right>
                                    </Right>
                                </View>
