...
import {Text, View, StyleSheet, Alert, AsyncStorage, TextInput, ActivityIndicator} from 'react-native';
    ...
    _retrieveData = async () => {
        try {
          ...
          if (token !== null) {
            ...
            var temp="https://afandim.net/merchantapp/api/GetTodaysOrder?token="+this.state.user_token+"&user_type="+this.state.user_info['user_type']+"&mtid="+this.state.user_info['merchant_id']+"&lang=en&api_key=2bf5c1669378eeda07ed05b3a8e574ef&app_version=2.5&token="+this.state.user_token+"&user_type="+this.state.user_info['user_type']+"&mtid="+this.state.user_info['merchant_id']+"&device_platform=Android&json=true";
                                ...
                                <View style={{flexDirection:'row'}}>
                                    ...
                                    <Right>
                                        <View style={{flexDirection:'row', marginRight:20}}>
                                            <Icon name="stopwatch" type="Entypo" style={{color:'red', fontSize:15}}></Icon>
                                            <Text note style={{color:'red', fontSize:13}}>{offer.time}</Text>
                                        </View>
                                    </Right>
                                </View>
                            </Body>
